...
A launchd job on Vlad's Mac (`scripts/push-unit-one.sh`, every 2 min): 5. Fires only when `.deploy-request` differs from `.deploy-done`, so Vlad's manual edits to `tracker.html` are never auto-committed. 6. Commits `tracker.html` as `unit-one-agent`, pushes to `main`. 7. Push to `main` auto-deploys to Vercel.
...
- Statuses are localStorage, per browser+origin: they don't sync between your local file, the live site, other devices, or visitors — each store is independent. Setting a status on the live site saves only to that one browser. Sharing/persisting them needs a backend (tracked in Linear, update this file + Linear once implemented).
